...
export const getAllPosts=async(req,res)=>{
  try{                             //latest post at top
    const posts=await Post.find().sort({createdAt:-1}).populate({
      path: "user",
      select: "-password",
    })//populate("user").select("-password") seems usable but cant be used as select("-password") doesnt work
    // with populate
    .populate({
      path: "comments.user",
      select: "-password",
...
export const getFollowingPosts=async(req,res)=>{
  ...
  const followingPosts=await Post.find({user: followingUsers}).sort({createdAt:-1})
  .populate(
    {
      path: "user",
      select:"-password",
